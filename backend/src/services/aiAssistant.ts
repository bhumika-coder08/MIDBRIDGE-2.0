...
export async function assembleUserContext(userId: string): Promise<AssistantContext> {
...
export async function generateAssistantResponse(prompt: string, context: AssistantContext): Promise<string> {
...
Give a structured, clear, and actionable response with markdown bullets. Do not make up non-existent government requirements.`
  ...
  if (q.includes('cost') || q.includes('expense') || q.includes('budget') || q.includes('gap') || q.includes('price') || q.includes('fee')) {
    ...
    for (const it of costItems) {
      ...
      if (it.timing === 'PRE_DEPARTURE') pre += amt;
      else if (it.timing === 'FIRST_MONTH') first += amt;
      else if (it.timing === 'MONTHLY_RECURRING') monthly += amt;
