...
export async function calculateJourneyReadiness(journeyId: string): Promise<ReadinessReport> {
  ...
  const userReqsResult = await query(
    `SELECT ur.*, r.category, r.title, r.mandatory, r.stage_number
  ...
  const satisfiedHealthKeys = standardHealthKeys.filter(cat =>
  ...
  await query(
    `UPDATE journeys SET readiness_score = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2`,
    [overall, journeyId]
  );
