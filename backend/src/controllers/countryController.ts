...
export async function getCountryByCode(req: Request, res: Response): Promise<void> {
  try {
    ...
    const countryRes = await query(
      `SELECT * FROM countries WHERE LOWER(code) = LOWER($1) OR LOWER(name) = LOWER($1)`,
      [code]
    );
    ...
    const contentRes = await query(
      `SELECT category, title, content, source_organization, source_url, last_checked
    ...
    const schRes = await query(
      `SELECT * FROM scholarships WHERE LOWER(country_code) = LOWER($1) ORDER BY deadline ASC`,
      [country.code]
    );
