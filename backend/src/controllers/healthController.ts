...
export async function getHealthDocuments(req: Request, res: Response): Promise<void> {
  try {
...
export async function uploadHealthDocument(req: Request, res: Response): Promise<void> {
  try {
    const userId = req.user!.id;
    const file = req.file;
...
export async function downloadHealthDocument(req: Request, res: Response): Promise<void> {
  try {
    const { id } = req.params;
    const userId = req.user!.id;
    ...
    if (!isOwner) {
      ...
      const shareCheck = await query(
        `SELECT hsp.id FROM health_share_packages hsp
         JOIN health_share_documents hsd ON hsp.id = hsd.health_share_id
         WHERE hsd.health_doc_id = $1 AND hsp.is_revoked = false AND hsp.expires_at > CURRENT_TIMESTAMP`,
        [id]
      );
...
export async function deleteHealthDocument(req: Request, res: Response): Promise<void> {
  try {
    const { id } = req.params;
    const userId = req.user!.id;
...
export async function getHealthReadiness(req: Request, res: Response): Promise<void> {
  try {
    const userId = req.user!.id;
    ...
    const docsRes = await query(
      `SELECT category, status, expiry_date FROM health_documents WHERE user_id = $1`,
      [userId]
    );
...
export async function createHealthSharePackage(req: Request, res: Response): Promise<void> {
  try {
    const userId = req.user!.id;
    ...
    const checkDocs = await query(
      `SELECT id FROM health_documents WHERE user_id = $1 AND id = ANY($2::text[])`,
      [userId, healthDocIds]
    );
    ...
    await query(
      `INSERT INTO health_share_packages (id, user_id, share_token, recipient_name, recipient_email, allow_download, expires_at, notes)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
      [packageId, userId, shareToken, recipientName, recipientEmail || null, !!allowDownload, expiresAt, notes || null]
    );

    for (const docId of healthDocIds) {
      await query(
        `INSERT INTO health_share_documents (id, health_share_id, health_doc_id) VALUES ($1, $2, $3)`,
        [uuidv4(), packageId, docId]
      );
    }
...
export async function getPublicHealthSharePackageByToken(req: Request, res: Response): Promise<void> {
  try {
    const { token } = req.params;

    const pkgRes = await query(
      `SELECT hsp.*, p.full_name as user_name, p.nationality
       FROM health_share_packages hsp
       JOIN users u ON hsp.user_id = u.id
       LEFT JOIN profiles p ON u.id = p.user_id
       WHERE hsp.share_token = $1`,
      [token]
    );
    ...
    const docsRes = await query(
      `SELECT hd.id, hd.category, hd.title, hd.original_name, hd.file_size, hd.file_hash, hd.status, hd.issuer, hd.issue_date, hd.expiry_date, hd.created_at
       FROM health_documents hd
       JOIN health_share_documents hsd ON hd.id = hsd.health_doc_id
       WHERE hsd.health_share_id = $1`,
      [pkg.id]
    );
