...
export async function getReceivedApplications(_req: Request, res: Response): Promise<void> {
  try {
    // Return share packages directed to institutions/admissions or all packages for university role review
    const result = await query(
      `SELECT sp.*, u.email as applicant_email, p.full_name as applicant_name, p.nationality, p.destination_country, p.intended_course,
