...
export async function createSharePackage(req: Request, res: Response): Promise<void> {
  try {
    ...
    const userDocs = await query(
      `SELECT id FROM documents WHERE user_id = $1 AND id = ANY($2::text[])`,
      [userId, documentIds]
    );
    ...
    await query(
      `INSERT INTO share_packages (id, user_id, share_token, recipient_name, recipient_email, allow_download, expires_at, notes)
    ...
    for (const docId of documentIds) {
      const linkId = uuidv4();
      await query(
        `INSERT INTO share_package_documents (id, share_package_id, document_id) VALUES ($1, $2, $3)`,
        [linkId, packageId, docId]
      );
    }
...
export async function getUserSharePackages(req: Request, res: Response): Promise<void> {
  try {
    const userId = req.user!.id;
    const pkgsRes = await query(
      `SELECT sp.*, COUNT(spd.document_id) as document_count
...
export async function revokeSharePackage(req: Request, res: Response): Promise<void> {
  try {
    const { id } = req.params;
    const userId = req.user!.id;

    await query(
      `UPDATE share_packages SET is_revoked = true WHERE id = $1 AND user_id = $2`,
      [id, userId]
    );
...
export async function getPublicSharePackageByToken(req: Request, res: Response): Promise<void> {
  try {
    const { token } = req.params;

    const pkgRes = await query(
      `SELECT sp.*, p.full_name as user_name, p.nationality
       FROM share_packages sp
    ...
    const docsRes = await query(
      `SELECT d.id, d.category, d.original_name, d.file_size, d.file_hash, d.verification_status, d.issuer, d.issue_date, d.expiry_date, d.created_at
       FROM documents d
       JOIN share_package_documents spd ON d.id = spd.document_id
       WHERE spd.share_package_id = $1`,
      [pkg.id]
    );
