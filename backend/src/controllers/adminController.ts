...
export async function getAllUsers(_req: Request, res: Response): Promise<void> {
  try {
    const result = await query(
      `SELECT u.id, u.email, u.role, u.created_at, p.full_name, p.nationality, p.destination_country, p.purpose
       FROM users u
...
export async function addCountryRequirement(req: Request, res: Response): Promise<void> {
  try {
    ...
    await query(
      ...
      [id, null, destination.toUpperCase(), purpose, category, title, description || '', mandatory !== false, stageNumber || 5, sourceUrl || null]
    );
