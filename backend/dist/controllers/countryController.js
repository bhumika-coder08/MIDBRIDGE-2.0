...
export async function getAllCountries(_req, res) {
    try {
        const result = await query(`SELECT code, name, region, flag_emoji, cover_image, popular_purposes, summary, processing_time_weeks, currency, language
       FROM countries
       ORDER BY name ASC`);
...
export async function getCountryByCode(req, res) {
    try {
        ...
        const countryRes = await query(`SELECT * FROM countries WHERE LOWER(code) = LOWER($1) OR LOWER(name) = LOWER($1)`, [code]);
        ...
        const contentRes = await query(`SELECT category, title, content, source_organization, source_url, last_checked
       FROM country_content
       WHERE LOWER(country_code) = LOWER($1)
       ORDER BY category ASC`, [country.code]);
        // Fetch associated scholarships
        const schRes = await query(`SELECT * FROM scholarships WHERE LOWER(country_code) = LOWER($1) ORDER BY deadline ASC`, [country.code]);
        res.json({
            country,
