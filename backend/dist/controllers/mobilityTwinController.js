...
async function getMobilityTwin(req, res) {
    try {
       ...
       ORDER BY r.stage_number ASC`, [journey.id]);
        ...
        const satisfiedHealth = standardHealthCategories.filter(cat => healthDocs.some(hd => hd.category === cat && (!hd.expiry_date || new Date(hd.expiry_date) > new Date())));
        ...
        const overallReadiness = Math.round(docReadiness * 0.25 +
