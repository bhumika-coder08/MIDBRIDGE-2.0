...
export async function getReminders(req, res) {
    try {
        ...
        const result = await query(`SELECT * FROM reminders WHERE user_id = $1 ORDER BY due_date ASC`, [userId]);
...
export async function toggleReminder(req, res) {
    try {
        const { id } = req.params;
        const userId = req.user.id;
        await query(`UPDATE reminders SET is_completed = NOT is_completed WHERE id = $1 AND user_id = $2`, [id, userId]);
