...
export async function getActiveJourney(req, res) {
    try {
        const userId = req.user.id;
        const journeysRes = await query(`SELECT * FROM journeys WHERE user_id = $1 ORDER BY updated_at DESC LIMIT 1`, [userId]);
        ...
        // Fetch 12 stages
        const stagesRes = await query(`SELECT * FROM journey_stages WHERE journey_id = $1 ORDER BY stage_number ASC`, [journey.id]);
        // Fetch requirements & user checklist status
        const reqsRes = await query(`SELECT ur.id as user_requirement_id, ur.status, ur.notes, ur.updated_at,
       ...
       ORDER BY r.stage_number ASC, r.category ASC`, [journey.id]);
        // Fetch documents
        const docsRes = await query(`SELECT id, requirement_id, category, original_name, file_size, mime_type, file_hash, verification_status, issuer, expiry_date, created_at
       FROM documents
...
export async function createJourney(req, res) {
    try {
       ...
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`, [journeyId, userId, fromCountry, destName, purpose, 1, 'Researching', 10, notes || null]);
        ...
        await query(`UPDATE profiles SET current_country = $1, destination_country = $2, purpose = $3, updated_at = CURRENT_TIMESTAMP WHERE user_id = $4`, [fromCountry, destName, purpose, userId]);
        await logAuditEvent({
            userId,
...
export async function updateJourneyStage(req, res) {
    try {
        const { journeyId, stageNumber } = req.params;
        const { status } = req.body; // 'NOT_STARTED', 'IN_PROGRESS', 'COMPLETED', 'NEEDS_ATTENTION'
        await query(`UPDATE journey_stages SET status = $1, updated_at = CURRENT_TIMESTAMP
       WHERE journey_id = $2 AND stage_number = $3`, [status, journeyId, parseInt(stageNumber, 10)]);
        ...
        if (status === 'COMPLETED') {
            ...
            await query(`UPDATE journeys SET current_stage_number = $1, current_stage_name = $2 WHERE id = $3`, [nextNum, nextName, journeyId]);
        }
        const readiness = await calculateJourneyReadiness(journeyId);
...
export async function updateRequirementStatus(req, res) {
    try {
        ...
        await query(`UPDATE user_requirements SET status = COALESCE($1, status), notes = COALESCE($2, notes), updated_at = CURRENT_TIMESTAMP WHERE id = $3`, [status, notes, userRequirementId]);
        const readiness = await calculateJourneyReadiness(journeyId);
